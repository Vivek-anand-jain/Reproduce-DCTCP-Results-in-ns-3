...
void
TcpNewReno::UpdatePacingRate (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  ...
  if (tcb->m_cWnd < tcb->m_ssThresh/2)
    {      
      NS_LOG_DEBUG ("Pacing according to slow start factor; " << tcb->m_cWnd << " " << tcb->m_ssThresh);
      factor = static_cast<double> (tcb->m_pacingSsRatio)/100;
    }
   else
    {      
      NS_LOG_DEBUG ("Pacing according to cong. avoid. factor; " << tcb->m_cWnd << " " << tcb->m_ssThresh);
      factor = static_cast<double> (tcb->m_pacingCaRatio)/100;
    }
  Time lastRtt = tcb->m_lastRtt;  // cast to the underlying Time type
  NS_LOG_DEBUG ("Last RTT is " << lastRtt.GetSeconds ());
  DataRate pacingRate (static_cast<uint64_t> ((tcb->m_cWnd * 8 * factor) / lastRtt.GetSeconds ()));
  if (pacingRate < tcb->m_maxPacingRate)
    {
      NS_LOG_DEBUG ("Pacing rate updated to: " << pacingRate);
      tcb->m_currentPacingRate = pacingRate;
    }
  else
    {
