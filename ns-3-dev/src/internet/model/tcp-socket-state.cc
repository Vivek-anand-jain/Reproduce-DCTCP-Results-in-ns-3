...
TcpSocketState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketState")
    ...
    .AddAttribute ("PacingCaRatio", "Percent pacing rate increase for cong. avoid conditions",
                   UintegerValue (120),
                   MakeUintegerAccessor (&TcpSocketState::m_pacingCaRatio),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("PaceInitialWindow", "Perform pacing for first window of data",
                   BooleanValue (false),
